...
ebsd = ebsd(inpolygon(ebsd, [0 0 80 60]));
...
% % from https://mtex-toolbox.github.io/SlipSystemAlumina.html
% 
% [grains,ebsd.grainId] = calcGrains(ebsd,'angle',5*degree, 'alpha',1);
% 
% % all Corundum Corundum boundaries
% gB = grains.boundary('indexed');
% 
% % define the pseudo symmetry
% pseudoSym = orientation.byAxisAngle(grains.CS.cAxis,180*degree);
% 
...
% [~] = evalc("[grains, parentId] = grains.merge(gB(ispseudoBnd),'calcMeanOrientation',@(g) updateOri(g,pseudoSym));");
% 
...
figure;
plot(grains, grainColors, 'faceAlpha', 0.3)
hold on
...
traces_in_GrainCoords = inv(grains(sigmaGBS.grainId(:,1)).meanOrientation).*traces;

trace_in_maxgbcd = angle(traces_in_GrainCoords, gbcd_max_miller)/degree>70*degree;
...
plot(grains, grainColors, 'faceAlpha', 0.3)
hold on
plot(sigmaGBS, 'lineColor', 'b', 'lineWidth', 2, 'DisplayName', 'Inconsistent with GBCD max')
